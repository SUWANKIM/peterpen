...
public class MyAlertDialog extends Dialog {
    ...
    public void setTitleText(String title, int index, int start, int end) {
        ...
        for (int i = 0; i < arr.length; i++) {
            ...
            if (i == index) {
                ...
                spanText.setSpan(new ForegroundColorSpan(0xFF000000), 0, arr[i].length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
            if (i != arr.length - 1)
    ...
    public void setPointVisible(Boolean bShow) {
        if (bShow)
            findViewById(R.id.lly_point).setVisibility(View.VISIBLE);
        else
            findViewById(R.id.lly_point).setVisibility(View.GONE);
    }

    public void setHeartVisible(Boolean bShow){
        if (bShow)
            findViewById(R.id.lly_heart).setVisibility(View.VISIBLE);
        else
            findViewById(R.id.lly_heart).setVisibility(View.GONE);
    }
